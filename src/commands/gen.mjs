...
引数:
  files            生成対象の .md（省略時は adr/*.md すべて）

オプション:
  --check          生成せず正本との不整合だけ検出（CI 用、不整合があれば終了コード 1）
  -h, --help       このヘルプ`;
...
export async function run(argv = []) {
  ...
  if (check) {
    if (changed) {
      console.error(`\n${changed} 件の HTML が正本と不整合です。\`ema gen\` を実行してください。`);
      return 1;
    }
    console.log('  すべての HTML が正本と一致しています。');
    return 0;
  }
  console.log(`\n  ${wrote} 件生成（${list.length} 件中、差分なしはスキップ）。`);
  return 0;
}
